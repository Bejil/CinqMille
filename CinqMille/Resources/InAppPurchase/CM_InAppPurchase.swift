...
public class CM_InAppPurchase : Codable {
	
	static let shared = CM_InAppPurchase()
		...
		if capping ?? false {
			...
			CM_Alert_ViewController.presentLoading { alertController in
				...
				Task {
					...
					let products = await CM_InAppPurchase.shared.fetchProducts()
						...
						if let product = products.first(where: { $0.id == InAppPurchase.RemoveAds }) {
							...
							let buyButton = alertController.addButton(title: String(key: "inAppPurchase.purchase.alert.buy.button")) { button in
								...
								Task {
									...
									let success = await CM_InAppPurchase.shared.purchase(product: product)
									...
									alertController.close {
										...
										Task { @MainActor in
											...
											if success {
												...
												let alertController = CM_Alert_ViewController()
												...
												CM_Alert_ViewController.present(CM_Error(String(key: "inAppPurchase.purchase.error.alert.content")))
											}
										}
									}
							...
							let restoreButton = alertController.addButton(title: String(key: "inAppPurchase.purchase.alert.restore.button.title")) { button in
								...
								Task {
									...
									let restored = await CM_InAppPurchase.shared.restorePurchases()
									...
									alertController.close {
										...
										Task { @MainActor in
											...
											if restored.contains(InAppPurchase.RemoveAds) {
												...
												CM_Alert_ViewController.present(CM_Error(String(key: "inAppPurchase.restore.error.content")))
											}
										}
									}
								}
